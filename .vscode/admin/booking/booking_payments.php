...
<body onload="doConvert()" style="padding: 1rem;color: black;font-size:13px;margin:auto;font-family:-apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, 'Open Sans', 'Helvetica Neue', sans-serif">
  ...
  <section style="border-style:groove; border-width:thin;margin: auto; width: 1000px;height:1470px;border: 1px solid rgb(147 78 255);padding: 5px;">
      ...
      <tbody>
        ...
        while ($FetchAllPayments = mysqli_fetch_array($getpayments)) {
          ...
          if ($payment_mode == "check") {
            ...
            $payment_details = "<br>by check no: $checknumber issued on $check_issued_at for $checkissuedto through $bankName | $ifsc e.i $payment_status";
          } else if (
            ...
            $payment_details = "<br>by Online Banking : Bank Name:$OnlineBankName, TxnId: $transactionId, Details: $payment_details, Status: $payment_status";
          } else if (
            $payment_mode == "cash"
          ) {
            ...
            $payment_status = "done!";
            $payment_details = "<br>Cash " . $payment_amount . " is received by $cashreceivername on " . date("d M, Y h:m A", strtotime($paymentcreatedat));
          }
        ...
        ?>
          <tr class="striped">
            ...
            <td align="right" width='15%' style='font-size:10px;'><?php echo $payment_details; ?></td>
        ...
        ?>
        <tr>
          <td class="text-primary" align="right" COLSPAN="8"><span class="fs-16"><b>Rs.<?php echo $TotalPayment; ?></b></span></td>
    ...
    <table style="text-align:right;line-height: 14px;font-size:14px !important;width:100% !important;margin-top:1rem !important;font-weight:600 !important;">
      ...
      <tr>
        <th>Total Paid (<?php echo $PaidPercentage = round($TotalAmountPaid / $net_payable_amount * 100, 2); ?>%) :</th>
      ...
      ?>
        <tr>
          ...
          <td>Rs.<?php echo $PreviousPayment; ?></td>
        </tr>
      <?php } ?>
      <tr>
        <?php
        ...
        $TotalPaidAmount = 0;
        while ($FetchAllPayments = mysqli_fetch_array($GetPAYMENTS)) {
          $TotalPaidAmount += $FetchAllPayments['net_paid'];
        }
        ?>
        <th>Balance (<?PHP echo $Percentage = 100 - $PaidPercentage; ?>%) :</th>
        <td style="color:red;">Rs.<?php
        if($Percentage == 0){
            echo "0";
        } else {
        echo $net_payable_amount - $TotalPaidAmount - $PreviousPayment; 
        }
        ?></td>
      </tr>
    </table>
